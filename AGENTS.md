...
- `tools/auto_exec.py` — **T6.5 機械單執行器（v1 dry-run，2026-09-14）**：以**前一收盤**判定 R23 線 / 用戶裁決收盤線 / 選擇權管理線 / R24 SGOV / R8 缺口，套 R14 鎖、財報窗、T5.5 反轉，同標的同向去重（用戶線 > R23-30 > R23-20），寫 `briefing-out/cache/auto-exec-plan.json`；**briefing T6.5 只准執行此清單**，模型不再自行判定機械觸發。`--execute` 鎖至 2026-09-28（兩週乾跑對照後由用戶解鎖）。runner 預跑（前置 `position_guard --sync-alerts`）
